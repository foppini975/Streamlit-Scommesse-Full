...
def load_data():
    ...
    df.rename(columns={'refreshed_at': 'refreshed_at_string'}, inplace=True)
    df['refreshed_at'] = df.apply (lambda row: datetime.datetime.strptime(row['refreshed_at_string'],"%Y-%m-%d %H:%M:%S.%f").date(), axis=1)
...
def main():
    ...
    label_length = st.sidebar.slider('Label characters:', 1, 10, 3)
    ...
    df_filtered = df[(df['league'] == league_select) & (df['refreshed_at'] >= start_date)]
    st.table(df_filtered[['home_team', 'away_team', 'refreshed_at'] + betting_columns[betting_select]])
